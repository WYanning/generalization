...
class MultiTaskMappingDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        columns_to_delete = [6,7,8,9,10,11]
        ...
        # print(adjacency)
        return Data(pos_org=pos_org, pos=pos,x=x, edge_index=edge_index,edge_index_tri=edge_index_tri,adjacency=adjacency, updated_adjacency=updated_adjacency, node_movement=node_movement,mask=mask,
                    file_name=os.path.basename(index_folder),num_edges=num_edges,adj_tri=adjacencies_tri)


if __name__ == '__main__':
    save_loc_movement = r"D:\dresden\mapgeneralization\output0914\movement_muti_feature"
    save_loc_adj = r"D:\dresden\mapgeneralization\output0914\adj_muti_feature"
    ...
    in_channels_node= train_dataset[0].x.shape[1]+2
    in_channels_edge = train_dataset[0].x.shape[1]
    out_channels_node = 2
    ...
    model = GraphSageModel(in_channels_node, [64, 64, 64], out_channels_node,in_channels_edge,[64, 64, 64],out_channels_edge).to(device)
    ...
    for epoch in range(num_epochs):
        ...
        for batch in data_loader:
            ...
            mask = batch.mask
            output_node, updated_pos, output_edge = model(batch.x, batch.edge_index, batch.pos, mask, batch.batch, batch.adjacency, batch.adj_tri)
            loss = model.compute_loss(output_node, batch.node_movement, mask, batch.pos_org, batch.edge_index_tri, output_edge,batch.updated_adjacency, mask,
                                      batch.adjacency,  batch.adj_tri)
            acc = model.accuracy(output_edge, batch.updated_adjacency, mask, batch.adjacency)
            mse = model.mse(output_node, batch.node_movement, mask)
        ...
        avg_train_mse = train_mse_sum / len(data_loader)
        ...
        with torch.no_grad():
            for batch in valid_loader:
                batch = batch.to(device)  # 将数据移至设备（GPU或CPU）
                mask = batch.mask
                output_node, updated_pos, output_edge = model(batch.x, batch.edge_index, batch.pos, mask, batch.batch,
                                                              batch.adjacency, batch.adj_tri)
                loss = model.compute_loss(output_node, batch.node_movement, mask, batch.pos_org, batch.edge_index_tri,
                                          output_edge, batch.updated_adjacency, mask,
                                          batch.adjacency, batch.adj_tri)
                acc = model.accuracy(output_edge, batch.updated_adjacency, mask, batch.adjacency)
                mse = model.mse(output_node, batch.node_movement, mask)
                ...
                valid_mse_sum += mse.item()
        avg_valid_loss = valid_loss / len(valid_loader)
        avg_valid_acc = valid_acc / len(valid_loader)
        avg_valid_mse = valid_mse_sum / len(valid_loader)
        valid_losses.append(avg_valid_loss)
        valid_accuracy.append(avg_valid_acc)
        valid_mse.append(avg_valid_mse)
        print(
            f"Epoch [{epoch + 1}/{num_epochs}], Train Loss: {avg_train_loss:.4f}, Valid Loss: {avg_valid_loss:.4f}, Valid Acc: {avg_valid_acc:.4f},Mse:{avg_valid_mse:.4f}")
    ...
    model_save_path = 'D:\\dresden\\mapgeneralization\\output0914\\model'
    os.makedirs(model_save_path, exist_ok=True)  # 创建目录，如果已存在则不会抛出错误
    # 保存模型状态字典
    model_file_path = os.path.join(model_save_path, 'best_model_muti_feature.pth')
    ...
    plt.figure(figsize=(10, 5))
    plt.plot(train_mse, label='Train Acc', color='blue')  # 蓝色的实线
    plt.plot(valid_mse, label='Valid Acc', color='orange')  # 橙色的实线
    plt.title('MSE')
    plt.xlabel('Epoch')
    plt.ylabel('MSE')
    plt.legend()
    ...
    results_save_path = 'D:\\dresden\\mapgeneralization\\output0914\\results_muti_feature'
    os.makedirs(results_save_path, exist_ok=True)
    results_file_path = os.path.join(results_save_path, 'training_results.csv')
    df.to_csv(results_file_path, index=False)
    print(f"训练结果已保存到 {results_file_path}")

    model_file_path = 'D:\\dresden\\mapgeneralization\\output0914\\model\\best_model_muti_feature.pth'
    ...
    with torch.no_grad():
        for batch in test_loader:
            ...
            output_node, updated_pos, output_edge = model(batch.x, batch.edge_index, batch.pos, mask, batch.batch,
                                                          batch.adjacency, batch.adj_tri)
            data.pos = updated_pos.detach()
            # 计算损失、准确率和MSE
            # node_move_pred = einops.rearrange(output_node,"(B N) F -> B N F", N=40).detach().cpu().numpy()
            node_move_pred = output_node.detach().cpu().numpy()
            for pred_, n_ in zip(node_move_pred, name):
                pd.DataFrame(pred_).to_csv(os.path.join(save_loc_movement, n_ + ".csv"))

            initial_edges = batch.adjacency.view(64, 40, 40)
            _, predicted_changes = torch.max(output_edge, dim=-1)
