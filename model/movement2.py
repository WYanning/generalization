...
if __name__ == '__main__':
    save_loc_movement = r"D:\dresden\mapgeneralization\output0719\movement"
    ...
    in_channels= train_dataset[0].x.shape[1]+2
    out_channels_node = 2
    ...
    for epoch in range(num_epochs):
        ...
        for batch in data_loader:
            ...
            # print(batch.x.shape)
            # print(batch)  # 打印整个批次的数据结构
            # print(batch.num_graphs)  # 打印批次中包含的图数量
            # 将数据移至设备（GPU或CPU）
            ...
            mse = model.compute_mse(output_node, batch.node_movement, mask)
            loss.backward()
            optimizer.step()
        ...
        print(
            f"Epoch [{epoch + 1}/{num_epochs}], Train Loss: {avg_train_loss:.4f}, Valid Loss: {avg_valid_loss:.4f},Mse:{avg_valid_mse:.4f}")
    ...
    # 保存模型状态字典
    model_file_path = os.path.join(model_save_path, 'movement_model.pth')
    ...
    plt.figure(figsize=(10, 5))
    plt.plot(train_mse, label='Train MSE', color='blue')  # 蓝色的实线
    plt.plot(valid_mse, label='Valid MSE', color='orange')  # 橙色的实线
    plt.title('MSE')
    plt.xlabel('Epoch')
    plt.ylabel('MSE')
    plt.legend()
    ...
    model_file_path = 'D:\\dresden\\mapgeneralization\\output0719\\model\\movement_model.pth'
